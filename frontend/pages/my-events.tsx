import { useSession } from "next-auth/react";
import { useEffect, useState } from "react";
import { EEvent } from "../interfaces/event";
import axios from "axios";
import EventCard, { EventOptions } from "../components/EventCard/EventCard";
import { useRouter } from "next/router";
import { useUpdateEventContext } from "../context/UpdateEventContext";

export default function MyEvents() {
  const session = useSession();
  const router = useRouter();
  const [events, setEvents] = useState<EEvent[]>([]);
  const [failedFetch, setFailedFetch] = useState<boolean>(false);

  const [isUpdating, setIsUpdating] = useState(false);
  const [isDeleting, setIsDeleting] = useState(false);

  const { setEventToBeUpdated } = useUpdateEventContext();

  useEffect(() => {
    if (!session.data || session.status !== "authenticated") {
      router.push("/");
    }
  }, [session, router]);

  async function loadMyEvents() {
    axios({
      url: process.env.NEXT_PUBLIC_SERVER_BASE + "/api/events/my-events",
      method: "GET",
      headers: {
        Authorization: "Bearer " + (session.data as any).accessToken,
      },
    })
      .then((response) => {
        setEvents(response.data as EEvent[]);
      })
      .catch((err: any) => {
        console.log(err.toString());
        setFailedFetch(true);
      });
  }

  async function updateEvent(event: EEvent) {
    setEventToBeUpdated?.(event);
    router.push('/update-event');
    return Promise.resolve(true);
  }

  async function deleteEvent(event: EEvent) {
    try {
      setIsDeleting(true);
      await axios.delete(
        process.env.NEXT_PUBLIC_SERVER_BASE + "/api/events/delete/" + event.id,
        {
          headers: {
            Authorization: "Bearer " + (session.data as any).accessToken,
          },
        }
      );
    } catch (error: any) {
      console.log(error.toString());
    } finally {
      setIsDeleting(false);
      await loadMyEvents();
      return true;
    }
  }

  useEffect(() => {
    loadMyEvents();
  }, []);

  return (
    <div className="container mx-auto p-8">
      <div className="flex flex-row flex-wrap -mx-2">
        {events.map((e) => {
          return (
            <EventCard
              event={e}
              key={e.id}
              actionsAvailable={[EventOptions.Delete, EventOptions.Update]}
              onDelete={deleteEvent}
              onUpdate={updateEvent}
              disableActions={isDeleting || isUpdating}
            />
          );
        })}
      </div>

      {failedFetch && (
        <div
          className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative"
          role="alert"
        >
          <strong className="font-bold">Holy smokes!</strong>
          <span className="block sm:inline">
            Something seriously bad happened.
          </span>
          <span className="absolute top-0 bottom-0 right-0 px-4 py-3">
            <svg
              className="fill-current h-6 w-6 text-red-500"
              role="button"
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 20 20"
            >
              <title>Close</title>
              <path d="M14.348 14.849a1.2 1.2 0 0 1-1.697 0L10 11.819l-2.651 3.029a1.2 1.2 0 1 1-1.697-1.697l2.758-3.15-2.759-3.152a1.2 1.2 0 1 1 1.697-1.697L10 8.183l2.651-3.031a1.2 1.2 0 1 1 1.697 1.697l-2.758 3.152 2.758 3.15a1.2 1.2 0 0 1 0 1.698z" />
            </svg>
          </span>
        </div>
      )}
    </div>
  );
}
